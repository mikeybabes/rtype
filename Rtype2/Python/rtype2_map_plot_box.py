...
def draw_tile_color_box(img, x, y, tile_block, char_data, palette_data, palette_usage, flip_x=False, flip_y=False):
    ...
    for out_i, tile_i in enumerate(layout):
        ...
        # Fill the square with the corresponding palette color
        for dy in range(CHAR_HEIGHT):
            for dx in range(CHAR_WIDTH):
                img.putpixel((px + dx, py + dy), color + (255,))  # Use the color with full opacity (alpha = 255)
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        print("Usage: python rtype2_map_plot_box.py tile_map.bin tiles.bin characters.bin palette.bin output.png [--columns N] [--height N] [--debug debug.txt]")
        sys.exit(1)
