...
def load_palette(palette_data, palette_index):
    offset = palette_index * 48
    return [tuple(palette_data[offset + i*3:offset + i*3 + 3]) for i in range(16)]
...
def draw_tile(img, x, y, tile_block, char_data, palette_data, flip_x=False, flip_y=False):
    ...
    for out_i, tile_i in enumerate(layout):
        ...
        palette = load_palette(palette_data, palette_index)
        ...
        for dy in range(CHAR_HEIGHT):
            for dx in range(CHAR_WIDTH):
                ...
                if color_index == 0:
                    img.putpixel((px + dx, py + dy), (0, 0, 0, 0))
                else:
